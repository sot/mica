...
pytestmark = pytest.mark.skipif(
    not HAS_WEB_SERVICES, reason="CDA web services not available"
)
...
def test_get_archive_file_list():
    dat = get_archive_file_list(
        obsid=2365, detector="pcad", subdetector="aca", level=1, filetype="aspsol"
    )
    assert len(dat) == 2
...
def test_get_proposal_abstract():
    exp = {
        "abstract": (
            "We propose the Chandra-COSMOS survey which will provide an "
