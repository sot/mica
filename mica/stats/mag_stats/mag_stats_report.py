...
class MagStatsReport:
    def __init__(self, agasc_stats, obs_stats, directory='./mag_stats_reports'):
    ...
    def multi_star_html(self, sections={}, updated_stars=[], fails=[],
    ...
    def plot_agasc_id_single(self, agasc_id, obsid=None,
        ...
        for i, obsid in enumerate(obsids):
            ...
            if np.any((timeline['obsid'] == obsid) & ok & highlighted):
                s = plt.scatter(
                    timeline['index'][(timeline['obsid'] == obsid) & ok & highlighted & inside],
                ...
                _ = plt.scatter(
                    ...
                    s=10, marker='^', color='orange', label='Highlighted')
                _ = plt.scatter(
                    timeline['index'][(timeline['obsid'] == obsid) & ok & highlighted & outside_down],
            ...
            if np.any((timeline['obsid'] == obsid) & ok & ~highlighted):
                ...
                if i == 0:
                    marker_handles.append(s)
                _ = plt.scatter(timeline['index'][
                                    (timeline['obsid'] == obsid) & ok & (~highlighted) & outside_down],
                                bottom[(timeline['obsid'] == obsid) & ok & ~highlighted & outside_down],
    ...
    def plot_flags(self, telemetry, ax=None, obsid=None):
        if ax is None:
            ax = plt.gca()
        ...
        ax_dr.set_ylabel('dr')
        ax_dr.scatter(timeline['x'][all_ok & (dr < 10)], dr[all_ok & (dr < 10)], s=3, marker='.',
                      color='k')
        ax_dr.scatter(timeline['x'][all_ok & (dr >= 10)], dr[all_ok & (dr >= 10)], s=3, marker='^',
                      color='k')
        ax_dr.scatter(timeline['x'][~all_ok & (dr < 10)], dr[~all_ok & (dr < 10)], s=3, marker='.',
    ...
    def plot_set(self, agasc_id, args, telem=None, filename=None):
        ...
        if telem is None:
            telem = mag_stats.get_telemetry_by_agasc_id(agasc_id, ignore_exceptions=True)
            telem = mag_stats.add_obsid_info(
                telem,
